...
module.exports = {
    ...
    orderSuccess:(order)=>{
        return new Promise((resolve,reject)=>{
          db.get().collection(collection.CART_COLLECTION).deleteOne({user:objectId(order)}) 
          resolve() 
        })
    },
    ...
    getCop:(code,userId) => {
        ...
        console.log('coupon db',code);
        return new Promise((resolve,reject)=>{
          db.get().collection(collection.COUPON_COLLECTION).findOneAndUpdate(
            {
                couponCode:code
            },
            {
